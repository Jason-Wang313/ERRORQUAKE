...
absolute-rate prediction. Accuracy and severity distribution are
independent axes of model evaluation and must be reported together.
\end{abstract}
...
\emph{Accuracy improvement can worsen the severity profile of the
errors that remain.} As dense open-weight models grow from $3$B to
$36$B parameters, the error rate typically drops, but the \bvalue{}
of the residual error distribution drops faster
($\rho_s = -0.689$, $p = 0.006$, robust to leave-one-out in all
$14/14$ drops). At matched accuracy ($|\Delta\varepsilon| < 0.05$),
$30/210$ model pairs in our catalog have disjoint \sdi{} confidence
intervals. \emph{Accuracy and severity distribution are independent
axes of model evaluation and must be reported together.} This paper
...
\paragraph{Calibration as orthogonal axis.} Uncertainty calibration
and severity distribution are orthogonal axes of model quality. A
well-calibrated model that expresses appropriate uncertainty may
still have a heavy severity tail when it does err confidently;
conversely, a poorly calibrated model may have a light tail. Work
on confidence calibration \citep{guo2017calibration,kadavath2022language}
complements our contribution: combining the two axes would identify
models that are both calibrated and tail-light --- the safest
deployment profile.
...
judges when they agree within $1.0$; otherwise median-of-three
with a tiebreaker. Pre-tiebreak inter-judge agreement on the
$60{,}568$ records where both judges produced a score is
quadratic-weighted Cohen's $\kappa = 0.374$ (linear $\kappa = 0.285$,
Spearman $\rho = 0.375$, mean absolute disagreement $= 1.25$ on the
$0$--$4$ grid); the secondary judge call failed on a non-random
subset of small-model records, so per-model $\kappa$ comparisons
are biased and we report only the pooled value here
(per-model breakdown in Appendix~\ref{app:judges}). A $340$-item
manual audit additionally finds that $33.5\%$ of judge score-$2.0$
verdicts are overcalls (S2, \S\ref{sec:sensitivity}). All inference
uses an open-access inference API hosting the target models on
third-party GPU infrastructure; prompts and model version strings
...
produce longer, more detailed responses, which expose more surface
area for judge overcalling (Appendix~\ref{app:overcall} documents a
$33.5\%$ overcall rate at score $2.0$). We address this directly.
\emph{First}, mean response length is \emph{not} monotonically
ordered by parameter count: across the $13$ dense models for which
we recorded raw responses, Spearman correlation between
$\log_{10}(\text{params})$ and $\log_{10}(\text{mean words})$ is
$\rho_s = -0.080$ ($p = 0.80$). Concrete counter-examples include
\textsc{gemma-2-27b} ($27$B, mean $16.4$ words, the third-shortest
in the catalog) and \textsc{ministral-14b} ($14$B, mean $72.6$
words, the longest); a verbosity confound that explains the headline
would require length to track size, and it does not.
\emph{Second}, we did observe that response length predicts
overcall rate ($\rho_s = +0.50$, $p = 0.05$), so verbose responses
\emph{are} more likely to be overcalled --- but because length is
not size-correlated, this routes the overcall noise into a
size-orthogonal direction. \emph{Third}, OLS regression of \sdi{} on
$\log_{10}(\text{params})$ controlling for $\log_{10}(\text{mean
words})$ on the $n=13$ dense subset gives
$\beta_{\text{params}} = -0.381$ ($\text{SE} = 0.126$, $p = 0.013$);
the headline scaling effect persists at $p < 0.05$ after the
verbosity covariate is partialed out. The univariate Spearman on the
$n=13$ subset is $\rho_s = -0.663$ ($p = 0.014$), comparable to the
$n=14$ headline. The Spearman correlation is also rank-based and
therefore robust to monotone score inflation provided the inflation
is approximately uniform across models. Per-model output token
counts and the partial-regression details are in
\texttt{results/analysis/verbosity\_check.json}; controlling for
length more aggressively (matched-length re-evaluation under a
truncated generation budget) is priority follow-up work.
...
\paragraph{S5: $m_{\min}$ sensitivity (post-hoc, reviewer
follow-up).} The headline scaling correlation is sensitive to the
choice of $m_{\min}$. Under our default model-specific KS-selector
(Method, \S\ref{sec:method}), $\rho_s = -0.689$ ($p = 0.006$,
$n_{\text{dense}} = 14$): larger dense models have shallower upper-tail
slopes (heavier upper tails). Under \emph{fixed $m_{\min} = 1.5$},
$\rho_s = +0.837$ ($p = 0.0002$); under the Clauset-style criterion
of $\geq 100$ exceedances (which selects $m_{\min} = 0.5$ for every
model), $\rho_s = +0.793$ ($p = 0.0007$). \emph{The sign flips.} The
two strategies estimate different objects: the auto-selector targets
the upper-tail slope at each model's informative range (and is the
appropriate estimator for catastrophic-tail claims), while the fixed
$m_{\min}$ strategies estimate the bulk decay rate over the full
positive grid (where most of the mass lives). The combined picture
is internally consistent: \emph{larger dense models commit fewer
small slips (steeper bulk decay) and a relatively higher fraction
of catastrophic fabrications (shallower upper-tail slope)}.
\textbf{The headline of this paper applies specifically to the
upper-tail slope as estimated by the auto-selector; the bulk slope
moves the opposite way.} See Appendix~\ref{app:s5} for the full
$3 \times 14$ table.
...
\paragraph{Judge overcalling.} A single-rater audit of $340$
score-$2.0$ judgements across $17$ models classifies $33.5\%$ as
overcalls, $47.9\%$ as genuine, and $18.5\%$ as ambiguous. The
bootstrap overcall correction (S2) leaves the \sdi{} ranking at
$\rho_s = 0.847$, $0.003$ below the $0.85$ threshold. The
\emph{headline} negative-scaling finding is robust to correction,
but readers who wish to take the absolute \sdi{} values or the
deployment-table numbers as ground truth should account for this.
Verbose models receive more overcalls, producing a response-style
confound that is characterised in Appendix~\ref{app:overcall}.

\paragraph{Scale resolution.} Sensitivity S1 shows that collapsing
the $9$-point severity grid to $7$ points or $5$ levels destroys
the \sdi{} ranking ($\rho_s = 0.43$ and $0.16$). Practitioners
must use the full $9$-point scale; coarser human-rating protocols
will not reproduce our results.

\paragraph{Model coverage.} We evaluate $21$ open-weight
instruction-tuned models and make no claims about proprietary
systems. Frontier-dense coverage is further constrained:
\textsc{llama-3.1-405b-instruct}, \textsc{gpt-oss-120b}, and
\textsc{minimax-m2.5} were excluded after rate-limit exhaustion on
the inference API. The largest dense model we evaluate is
$36$B. The scaling finding covers a $\sim 10\times$ range in
active parameters and should not be extrapolated to the $100$B+
dense regime without direct measurement. Three reasoning-specialised
models were also excluded after chain-of-thought truncation at a
$500$-token budget. Our seven MoE models span $3$--$37$B active
parameters and are too few to fit a separate scaling curve; the
MoE subset shows the same sign as dense ($\rho_s = -0.595$,
$p = 0.159$, $n=7$) but does not reach significance.

\paragraph{Human validation scope.} The $340$-item human-scored
subset was rated by a single expert rater; inter-rater ICC was
not formally computed. Proper multi-rater validation is future
work.

\paragraph{Absolute-rate prediction.} Experiment 3 fails its
primary pre-registered criterion ($\rho_s \geq 0.75$ at $M \geq
3$). We do not claim calibrated prediction --- only moderate rank
signal ($\rho_s = 0.443$, $p = 0.044$).

\paragraph{Potential for misuse.} A severity benchmark can be
optimised against. Because the $9$-level scale, the scoring
pipeline and the benchmark queries are released openly, gaming is
detectable: an adversarially fine-tuned model will not match the
...
the upper tail
($\rho_s(\log_{10}\text{params},\, \sdi) = -0.689$, $p = 0.006$,
$n_{\text{dense}} = 14$, leave-one-out robust in $14/14$). The sign
is specific to the upper-tail estimator; under fixed $m_{\min}$ that
fits the bulk of the positive grid the sign flips, indicating that
larger dense models commit fewer small slips and a relatively higher
fraction of catastrophic ones. Supporting this claim are two
foundational results --- $17/21$ models have a non-exponential
best-fit family by BIC and $18/21$ are Vuong-decisive ($p<0.05$),
and the \sdi{} discriminates $30$ model pairs that the error rate
treats as equivalent --- and one partial extension: micro-error
\sdi{} rank-orders catastrophic counts at $\rho_s = 0.44$
($p = 0.044$) but does not produce calibrated absolute-rate
predictions. The headline recommendation is methodological:
\emph{accuracy and severity distribution are independent axes of
model evaluation and must be reported together.}
