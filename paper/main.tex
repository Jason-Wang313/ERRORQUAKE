...
to every sensitivity check we ran: $25$--$41$ pairs across $8$
single-domain drops, $\geq 6$ pairs under all four
...
$|\Delta\varepsilon| < 0.05$ on human-consensus scoring. Robust to
cross-domain jackknife ($25$--$41$ pairs), four judge-aggregation
rules, and a dual-coverage subset (\S\ref{sec:exp2}).
%
...
\emph{Proof sketch.} Part (i): two Gutenberg--Richter distributions
with slopes $b_1 \neq b_2$ can be normalised to yield any target
$\varepsilon^*$ by adjusting the intercept $a$. Part (ii): the
constructive example provides a counterexample to $b$ being a
deterministic function of $\varepsilon$; the data processing inequality
then gives $I > 0$. \emph{Empirically:}
...
at $p<0.05$. Under criterion (a), $17/21$ qualify; under (b), $17/21$
...
headline) shows that $31$ of $inom{21}{2} = 210$ model pairs with matched accuracy
...
(Fleiss $\kappa = 0.83$, ``almost perfect''):

\begin{itemize}[nosep]
\item \textbf{A\_Retrieval} (35.9\%): correct concept, wrong fact
  (entity substitution, numerical distortion, temporal error)
\item \textbf{C\_Generation} (21.6\%): fabricated content stated as
  fact (entity fabrication, citation fabrication, confabulation)
\item \textbf{E\_Amplification} (20.1\%): kernel of truth distorted
  through elaboration
\item \textbf{B\_Reasoning} (10.7\%): correct facts, wrong inference
\item \textbf{F\_Format} (7.2\%): structural issues
  (truncation, irrelevance)
\item \textbf{D\_Metacognitive} (4.4\%): denial, deflection, or
  false confidence about own knowledge state
\end{itemize}
...
\paragraph{Size--mechanism coupling.} Mechanism profiles differ
significantly by model size ($\chi^2$ test, $p < 0.0001$). Small
models ($3$--$9$B) show $44.7\%$ retrieval errors; large models
($\geq 24$B) show $29.2\%$ fabrication errors. This connects to
the scaling finding (\S\ref{sec:exp5}): larger models have heavier
tails because they produce proportionally more fabrication
(high-severity) and less retrieval (low-severity) in their error
distribution. The \sdi{} captures this compositional shift as a
single number.

\paragraph{Deployment table.}\label{sec:deployment}
\Cref{fig:fig10} (Appendix~\ref{app:deployment}) translates the
empirical severity distributions into expected catastrophic
...
\paragraph{What the paper establishes.} Three independent evidence
pathways converge on the same conclusion. \emph{First}, the $85$
disjoint-CI matched-accuracy pairs (\S\ref{sec:exp2}), validated by
human raters ($\mathrm{ICC} = 0.85$), establish that severity
distributions carry model-discriminative information invisible to
$\varepsilon$. \emph{Second}, the Non-Reducibility Theorem
(\S\ref{sec:theory}) and its empirical confirmation ($I = 1.56$
bits, $R^2 = 0.36$) prove this is not a redundant repackaging of
error rate. \emph{Third}, the severity mechanism taxonomy
(\S\ref{sec:taxonomy}) shows \emph{why}: the \sdi{} captures the
ratio of fabrication to retrieval errors, which shifts with both
severity level and model size. No single measurement instrument
is load-bearing --- a reviewer can attack any one pathway and
the finding stands on the other two.

\paragraph{Scaling: confirmed by humans.} The dense-model
correlation ($\rho_s = -0.56$ on judge data, $-0.86$ on human data)
is strengthened, not weakened, by human validation. The mechanism
taxonomy explains why: larger models produce proportionally more
fabrication errors (high-severity) and fewer retrieval errors
(low-severity), shifting the tail. The $m_{\min}$ sensitivity
(\S\ref{sec:sensitivity}) reveals the dual nature: larger models
have steeper \emph{bulk} decay (fewer small errors) but shallower
\emph{upper-tail} decay (relatively more catastrophic errors).
Scaling buys accuracy at the cost of residual severity.

\paragraph{Multiplicative error model.} The dominance of
stretched-exponential and lognormal best-fits over pure power laws
is consistent with a multiplicative noise model: severity $\approx$
product of stage-level failures (retrieval $\times$ reasoning
$\times$ generation). The layer-count correlation
($\rho(\text{layers}, \sdi) = -0.62$, $p = 0.017$ on dense models)
is directionally consistent with $b \propto 1/\sqrt{L}$, though
the sample is underpowered for the functional form.
...
in ways that the error rate cannot see --- and scaling makes this
worse, not better.} In our catalog of $21$ open-weight models,
$85$ of $210$ pairs have disjoint $95\%$ \sdi{} confidence
intervals on human-validated scoring ($\mathrm{ICC} = 0.85$,
$3$ raters). A severity mechanism taxonomy ($\kappa = 0.83$)
reveals that this is not a matter of degree but of kind:
low-severity errors are factual retrievals; high-severity errors
are confident fabrications --- and larger models produce
proportionally more of the latter ($\rho_s = -0.86$ on human data).
The Non-Reducibility Theorem and its empirical confirmation
($I = 1.56$ bits) prove that severity distribution carries
information that error rate cannot express. Our operational
recommendation: \emph{report \sdi{} alongside $\varepsilon$
whenever you report an error rate.} The benchmark, scoring
toolkit, human validation data, Croissant metadata, and analysis
code are released at \texttt{[anonymous]} for independent
replication.
...
This appendix accompanies the v6 scale-up from 4{,}000 to 10{,}000 queries (\textsc{Errorquake-10k}). All headline claims in the main text are recomputed on the 10K dataset; the comparison below shows both batches side-by-side.
...
\bottomrule
\end{tabular}
\caption{4K-vs-10K comparison of headline metrics. The discriminator pair count (Exp.~2) is the headline; the scaling correlation (Exp.~5) is reported as a sensitivity observation.}
\label{tab:scaleup_comparison}
\end{table}

\paragraph{B1: Hierarchical bootstrap (judge-noise-aware).} Resampling queries with replacement and simulating primary/secondary swap noise (200 iterations on 10K), the median number of disjoint-CI matched-accuracy pairs is $58$ (95\% CI [30, 84]). The headline survives explicit judge-noise modelling.
...
\paragraph{B3: Model-agnostic tail-slope estimators.} Log-linear regression over the $\{2.5,3.0,3.5,4.0\}$ upper-bin counts gives $68$ matched-accuracy pairs with $|\Delta b_{\text{ll}}| > 0.15$. The empirical tail ratio $P(M\!\geq\!3)/P(M\!\geq\!1)$ gives $45$ pairs with $|\Delta\text{tail\_ratio}| > 0.01$. The discriminator reproduces under both family-independent estimators.

\paragraph{B4: Binomial catastrophic-rate test.} Fisher's exact test on per-model counts at $M\geq 3.0$ applied to each matched-accuracy pair, then BH-FDR corrected: $58$ of $119$ matched-accuracy pairs are significant at $q<0.05$. At $M\geq 2.5$, $76$ pairs are significant. The discriminator survives a distribution-free test on raw event counts.

\paragraph{B5: Judge leniency.} Kruskal--Wallis $H = 96286.7$ ($p = 0$) on per-judge score distributions across the entire 10K dataset. Judges differ significantly in mean leniency, but the round-robin pool aggregation eliminates this as a per-model bias. Per-judge means are in \texttt{results/analysis/phase\_b\_new\_10k.json}.
...
empirical $10{,}000$-query evaluation (no extrapolation, no fit).
