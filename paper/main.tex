...
have non-exponential BIC-best fits and $17/21$ are Vuong-decisive
...
($\rho_s(\log_{10}\text{params}, \sdi) = -0.562$) is reported as a
sensitivity analysis only: it weakens to $-0.20$ after partialing
...
pairs in our 21-model catalog, \textbf{$31$ have disjoint $95\%$
...
\textbf{C1} A 9-level ordinal severity scale with
...
\textbf{C2} \textsc{Errorquake-10k}: $10{,}000$ queries $\times$
...
non-exponential BIC-best fit, $17/21$ are Vuong-decisive at
...
correlation ($\rho_s(\log_{10}\text{params}, \sdi) = -0.562$,
$p = 0.006$, $n_{\text{dense}} = 14$) is reported with full
disclosure of its limitations: the sign weakens under
partial-correlation control for accuracy ($-0.20$, $n.s.$), under
...
$|\Delta b| > 0.15$. Of those, \textbf{$31$ have disjoint $95\%$
...
is $\rho_s = -0.562$ ($p = 0.006$; \Cref{fig:fig6}). The sign is
...
($\rho_s(\log_{10}\text{params}, \sdi) = -0.562$) is consistent
