...
\newcommand{\sdi}{$b$} % severity distribution index (shorthand)
...
% =====================================================================
\begin{abstract}
\textbf{Larger dense open-weight language models concentrate their
remaining errors toward catastrophic severity.} Hallucination
benchmarks report a scalar error rate and treat all errors as
equivalent, ignoring that a wrong date and a fabricated court ruling
differ by orders of magnitude in consequence. We introduce
\textsc{Errorquake-4k}, a 4{,}000-query benchmark that scores each
response on a continuous 0--4 error-severity scale across 8 domains
and 5 difficulty tiers, and we fit per-model severity distributions
for 21 open-weight models spanning 3B to 37B active parameters.
For each model we estimate a \textbf{severity distribution index
(\sdi)} via the Gutenberg--Richter slope of the magnitude-frequency
relation. Our headline: across the 14 dense models,
$\rho_s(\log_{10}\mathrm{params},\,\sdi) = -0.689$ ($p = 0.006$) ---
larger dense models are more accurate but their residual errors are
more severe. Three supporting results. \emph{First}, severity
distributions are strongly heavy-tailed: 18/21 have a Vuong-decisive
best fit ($p<0.05$); the winning families are stretched exponential
(10), truncated power law (5), exponential (4), and lognormal (2);
no model is a clean power law. \emph{Second}, the \sdi{} adds
information beyond the error rate: of 210 model pairs, 30 have
disjoint 95\% \sdi{} confidence intervals at matched accuracy
($|\Delta\varepsilon|<0.05$), including \textsc{deepseek-v3.2} vs.\
\textsc{ministral-14b} at $\varepsilon=0.586$ and $\Delta\sdi=0.47$.
\emph{Third}, cheap micro-error measurements on easy-tier queries
rank-order catastrophic counts on hard-tier queries at Spearman
$\rho=0.44$ ($p=0.044$), a moderate signal that fails the
pre-registered $\rho\geq 0.75$ threshold for calibrated magnitude
prediction. Accuracy and severity distribution are independent axes
of model evaluation and must be reported together.
\end{abstract}
...
with a tiebreaker. A $340$-item manual audit reveals that
$33.5\%$ of judge score-$2.0$ verdicts are overcalls (S2,
\S\ref{sec:sensitivity}). All inference uses an open-access
...
\midrule
Exp.~1 (distributions) & heavy-tailed best fit $\geq 70\%$ & $21/21$ heavy-tailed & \textbf{PASS} \\
...
$^\dagger$S2 misses the threshold by $0.003$ and is discussed as
``borderline fail''.}\label{tab:preregistered}
\end{table}
...
\textbf{stretched\_exp (10)}, \textbf{truncated\_power\_law (5)},
...
appear in \Cref{fig:fig1,fig:fig2,fig:fig3}. \emph{Takeaway:} severity
distributions are a real, heavy-tailed object --- not a noisy
by-product of the error rate.
...
a separate trend that reaches $p<0.05$. (ii)
...
Larger dense instruction-tuned LLMs concentrate their remaining errors
toward catastrophic severity
($\rho_s(\log\text{params},\, b) = -0.689$,
$p = 0.006$, $n_{\text{dense}} = 14$). This is the central, surprising
claim of the paper: capability scaling under current training recipes
reduces the \emph{rate} of errors and simultaneously reshapes the
\emph{distribution} of the errors that remain toward heavier tails.
Supporting this claim are two foundational results --- severity
distributions are heavy-tailed across all 21 models ($18/21$
Vuong-decisive, none best-fit by a clean power law), and the
Gutenberg--Richter $b$-value discriminates 30 model pairs that the
error rate treats as equivalent --- and one partial extension:
micro-error \sdi{} rank-orders catastrophic counts at
$\rho_s = 0.44$ ($p = 0.044$) but does not produce calibrated
absolute-rate predictions. We take the rank-only signal as
the seed of a future-work programme in which an expanded benchmark
could tighten the distribution fits enough to support cross-regime
tail extrapolation. In the meantime, our headline recommendation is
methodological: \emph{accuracy and severity distribution are
...
\textsc{llama-3.1-8b-instruct}   & 1.001 & [0.923, 1.082] & 2.0 &  383 & 2429 & 0.607 & exponential \\
\textsc{kimi-k2-instruct}        & 1.041 & [0.948, 1.147] & 2.0 &  245 & 2460 & 0.615 & trunc.\ power law \\
\textsc{llama-3.2-3b-instruct}   & 1.046 & [0.942, 1.169] & 3.0 &  206 & 1750 & 0.450 & stretched exp \\
...
\textsc{llama-3.2-3b-instruct} have the highest catastrophic rate
